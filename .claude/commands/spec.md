...
Before starting the feature specification process, you must execute these preliminary steps:

### Repository and Branch Setup
1. **Identify Working Repository**
   - cd to the right project directory ex: cavos-service-sdk

2. **Create Feature Branch**
   - Get the next available issue number by checking existing issues: `gh issue list --state=all --limit=1000 | wc -l`
   - Create and switch to new branch with format: `feat/{short_name}` 
   - Use: `git checkout -b feat/{short_name}` (since gh doesn't have direct branch creation, use git for branch operations)

3. **Create GitHub Issue**
   - Create a new issue with title format: `feat: {feat_description}`
   - Include a comprehensive description based on the user's feature request
   - Use: `gh issue create --title "feat: {feat_description}" --body "{detailed_description}"`
   - The issue description should be substantial and explain the feature's purpose, scope, and expected benefits
